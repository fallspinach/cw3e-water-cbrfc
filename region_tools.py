...
def get_region_tools():
    ...
    # CBRFC region boundary
    cbrfc_style = dict(weight=4, opacity=1, color='gray', fillOpacity=0)
    cbrfc_bound = dl.GeoJSON(url='assets/cbrfc_5.0.pbf', format='geobuf', id='cbrfc-bound',
                             options=dict(style=ns('b120_style')),
                             hideout=dict(colorscale=['black'], classes=[0], style=cbrfc_style, colorProp='Area_SqMi'))
    # HUC4
    huc_style = dict(weight=1, opacity=1, color='#202020', fillOpacity=0)
    huc_bound = dl.GeoJSON(url='assets/huc4_cbrfc_0.5_tooltip.pbf', format='geobuf', id='huc-bound',
    ...
    side_content = html.Div([
                    ...
                    dbc.Row(html.Div([
                        ...
                        dbc.Collapse([dbc.Row([basin_tools, basin_popup_plots])], 
